...
会在 pets/<宠物id>/ 下生成 pet_config.json 和 resources 占位目录；
将动画帧放入 pets/<id>/resources/ 或修改 pet_config.json 中的路径指向你的资源。
"""
import os
import sys
import json
...
def main():
    ...
    for state in ("stand", "walk_left", "walk_right", "walk_up", "walk_down"):
        os.makedirs(os.path.join(resources_dir, state), exist_ok=True)
    ...
    config = {
        ...
        "animations": {
            "stand": ["resources/stand/0.png", "resources/stand/1.png"],
            "walk_left": ["resources/walk_left/0.png", "resources/walk_left/1.png"],
            "walk_right": ["resources/walk_right/0.png", "resources/walk_right/1.png"],
            "walk_up": ["resources/walk_up/0.png", "resources/walk_up/1.png"],
            "walk_down": ["resources/walk_down/0.png", "resources/walk_down/1.png"],
            "dragged": ["resources/stand/0.png", "resources/stand/1.png"]
        },
        "bubbleStyle": {},
        "slots": {}
    }
    ...
    print("")
    print("下一步:")
    print("  1. 将动画帧放入 pets/" + pet_id + "/resources/ 下对应状态目录（stand, walk_left 等），或")
    print("     在 pet_config.json 的 animations 中改为指向项目根目录的路径（如 resources/stand/0.png）")
    print("  2. 在 pet_config.json 中按需修改 character.systemPrompt、bubbleStyle、slots")
    print("  3. 运行 python main.py，在托盘「切换宠物」中选择 " + pet_name)
